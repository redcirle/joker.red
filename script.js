...
document.addEventListener('DOMContentLoaded', () => {
  // 1. 选中所有要模拟 hover 的按钮
  const buttons = document.querySelectorAll('.glass, .zebra-glass, .zebra-blue-glass, .purple-striped-button, .red-button');

  // 2. 模拟 hover 的函数
  function simulateHover() {
    buttons.forEach(btn => btn.classList.remove('hovering')); // 先清空
    const randomIndex = Math.floor(Math.random() * buttons.length);
    buttons[randomIndex].classList.add('hovering'); // ✅ 这里添加
  }

  // 3. 定时执行
  setInterval(simulateHover, 2000);
});